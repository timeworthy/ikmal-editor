// CommonJS mirror of the focus-mode presets. The desktop shell and the VS Code
// adapter run in Node hosts; the browser adapter is packaged as an ES module.
// Both implementations are held to the same behaviour by
// tools/focus_mode.test.mjs, which exercises every copy through one table.
//
//
...
const FOCUS_MODES = ['active', 'paused', 'zen'];
...
const ZEN_SENSITIVITY = 0;

function focusDuration(id) {
...
function resolveFocusState(value, now = Date.now()) {
...
function startFocusState(mode, durationID, now = Date.now()) {
...
function applyFocusState(preferences, value, now = Date.now()) {
...
function describeFocusState(value, now = Date.now()) {
  const state = resolveFocusState(value, now);
...
function confidenceThreshold(sensitivity) {
...
function filterMatches(matches, preferences) {
